...
bool King::isMoveValid(int new_x, int new_y)
{
...
bool Queen::isMoveValid(int new_x, int new_y)
{
	int delta_x = abs(_location.x - new_x);
	int delta_y = abs(_location.y - new_y);

	if ((delta_x > 0 && delta_y == 0) || (delta_x == 0 && delta_y > 0) || (delta_x > 0 && delta_y > 0 && delta_x == delta_y)){
		return true;
	}
	return false;
}
...
bool Rook::isMoveValid(int new_x, int new_y)
{
...
bool Bishop::isMoveValid(int new_x, int new_y)
{
	// Need to add special moves
...
bool Knight::isMoveValid(int new_x, int new_y)
{

	int delta_x = abs(_location.x - new_x);
	int delta_y = abs(_location.y - new_y);
	std::cout << delta_x << ", " << delta_y << std::endl;
	std::cout << new_x << " " << new_y << ", " << _location.x << " " << _location.y << std::endl;
...
bool Pawn::isMoveValid(int new_x, int new_y)
{
	...
	if ( (delta_x == 0 && delta_y == m*1) || (delta_x == 0 && delta_y == m*2 && first_turn) ){
		return true;
	}
	return false;
}
